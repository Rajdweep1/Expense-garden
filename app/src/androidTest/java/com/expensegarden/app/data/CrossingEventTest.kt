...
class CrossingEventTest {
    ...
    @Test fun breach_crossing_fires_once_not_on_every_subsequent_txn() = runBlocking {
        db.budgetDao().insert(BudgetEntity(categoryId = 1, month = month, amountPaise = 10_000, updatedAt = 1L)) // ₹100 on Food
    ...
    @Test fun raising_the_budget_can_legitimately_recross() = runBlocking {
        db.budgetDao().insert(BudgetEntity(categoryId = 1, month = month, amountPaise = 10_000, updatedAt = 1L))
        repo.saveManualLogged(draft(11_000), breachedAtLogging = true)     // 0 → 110 vs 100: cross #1
        db.budgetDao().deleteForCategory(1, month)
        db.budgetDao().insert(BudgetEntity(categoryId = 1, month = month, amountPaise = 20_000, updatedAt = 1L)) // raised to ₹200
    ...
    @Test fun confirm_path_fires_crossings_when_spend_materializes() = runBlocking {
        db.budgetDao().insert(BudgetEntity(categoryId = null, month = month, amountPaise = 10_000, updatedAt = 1L))
    ...
    @Test fun pace_warning_crossing_fires_without_breach() = runBlocking {
        ...
        db.budgetDao().insert(BudgetEntity(categoryId = null, month = month, amountPaise = 3_000_000, updatedAt = 1L))
