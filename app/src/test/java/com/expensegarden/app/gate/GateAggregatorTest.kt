...
class GateAggregatorTest {
    ...
    @Test fun `no scopes means OK and no offender`() {
        val v = GateAggregator.aggregate(emptyList(), 10_000L, 0L, 15, 30)
    ...
    @Test fun `category breach beats overall ok`() {
        ...
        val v = GateAggregator.aggregate(listOf(overall, food), 10_000L, 0L, 15, 30)
    ...
    @Test fun `overall breach alone still fires`() {
        val v = GateAggregator.aggregate(listOf(overall.copy(spentPaise = 995_000L)), 10_000L, 0L, 15, 30)
        assertEquals(Severity.BREACH, v.severity)
    ...
    @Test fun `at equal severity the deepest scope is named`() {
        ...
        val v = GateAggregator.aggregate(listOf(overall, parent, child), 2_000L, 0L, 15, 30)
        assertEquals(Severity.BREACH, v.severity)
    ...
    @Test fun `pace warning surfaces when nothing breaches`() {
        ...
        val v = GateAggregator.aggregate(listOf(overall.copy(spentPaise = 300_000L)), 100_000L, 0L, 10, 30)
